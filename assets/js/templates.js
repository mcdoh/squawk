...
export function squawkDisplayTMPL (data) {
	let template = `
		<p class="squawk-display" data-squawk="${ data.squawk }" data-expiration="${ data.expiration }">
			<strong><a href="${ data.squawk }">${ data.key }</a></strong>: <a href="${ data.url }">${ data.host || data.url }</a><span class="squawk-expiration"></span> <button type="button" class="btn btn-sm btn-info">Copy</button>
		</p>
	`;
